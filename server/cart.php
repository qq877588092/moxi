...
if($type == "add")
{
  ...
  $sql = "SELECT cart.*,goods.title,goods.src,goods.discount,goods.price FROM cart , goods WHERE cart.good_id = goods.good_id AND id=$id";
  ...
  for ($i = 0; $i < count($data); $i++) {
    /* 获取当前数据的店铺名称 */
     $currentShopName = $data[$i]["discount"];
